...
''' 
    ...
    within its own class. This pattern helps organize the logic by seperating each
    operation into its own class, adhering to the "Single Responsibility Principle" of OOP.
        ...
        - Value: Corresponding class (e.g., Addition class) that perform the operation.
'''
...
def main():
    ...
    while True:
        ...
        # Prompts the user for input and strip any extra whitespace.
        command = input('>>> ').strip()
        ...
        # If the user asks for help, print a list of availible commands.
...
""")
        # For all other inputs, attempt to process them as a calculation method.
        else:
            ...
            try:
                ...
                ''' 
                    Check if the operation is in the `operation_map`. This ensures we handle only valid operations.
                    This uses the LBYL (Look-Before-You-Leap) approach by checking if the operation exists before proceeding.
                '''

                if operation not in operations_map:
                    print(f"Unknown Operation '{operation}'. Type 'help' for instruction.")
            ...
            # Catch any other unforseen exceptions that may occur and display a generic error message.
